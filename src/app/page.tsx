...
export default function UnoGame() {
            ...
            case 'PLAY_CARD': {
                ...
                if (receivedUpdatedPlayers && Array.isArray(receivedUpdatedPlayers)) {
                    updatedPlayers = playersRef.current.map((existingPlayer) => {
                        ...
                        const displayName = receivedPlayer.id === myPlayerIdRef.current
            ...
            case 'ROUND_WINNER': {
                ...
                if (updatedPlayers && Array.isArray(updatedPlayers)) {
                    const updatedPlayersWithHands = playersRef.current.map(p => {
                        ...
                        if (updatedInfo) {
                            const displayName = p.id === myPlayerIdRef.current
                                ? `${updatedInfo.name} (You)`
                                : updatedInfo.name
    ...
    const initializeGameFromStart = useCallback(async (payload: any) => {
        ...
        console.log('Final players:', currentPlayers.map(p => ({
            id: p.id,
            name: p.name,
            position: p.position,
        ...
        channel.bind('player-left', (raw: unknown) => {
            ...
            setMpConnectedPlayers(prev => {
                const filtered = prev.filter(
                    p => p.id !== data.playerId && p.name !== data.playerName
                )
    ...
    const playCPU = useCallback(async (cpuId: Player['id']) => {
        ...
        if (chosenCard.value === 10) {
            ...
            if (drawPlayerIndex !== -1) {
                const drawPlayer = {
                    ...playersRef.current[drawPlayerIndex],
                    hand: [...playersRef.current[drawPlayerIndex].hand],
                }
            ...
            if (gameModeRef.current === 'multiplayer') {
                const minimalUpdatedPlayers = playersRef.current.map(p => {
                    ...
                    return {
                        ...
                        score: p.score,
                        position: p.position,
                        hand: (isCurrentPlayer || isDrawPlayer)
                            ? updatedPlayers.find(up => up.id === p.id)?.hand.map(c => ({
                                color: c.color,
                                value: c.value,
                                points: c.points,
